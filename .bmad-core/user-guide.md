...
**IMPORTANT**: BMad uses two locations for agent definitions:

1. **`.bmad-core/agents/`** - Source of truth for all agent definitions
2. **`.claude/commands/BMad/agents/`** - Slash command wrappers for Claude Code IDE

**Synchronization Process**:

When you update an agent definition in `.bmad-core/agents/`, you must sync it to `.claude/commands/BMad/agents/` for slash commands to work properly:

```bash
# Copy the updated agent definition
cp .bmad-core/agents/dev.md .claude/commands/BMad/agents/dev.md

# Then add the slash command header at the top if missing:
# /dev Command
#
...
**Why Two Locations?**
- `.bmad-core/` = Framework source of truth (maintained by BMad)
- `.claude/commands/` = Claude Code slash command integration (requires inline definitions)

**When to Sync**:
- After updating agent workflows or commands
- After adding new capabilities or dependencies
- When fixing agent behavior issues

**Verification**:
```bash
# Check if files are in sync
diff .bmad-core/agents/dev.md .claude/commands/BMad/agents/dev.md
```

If there are differences (besides the slash command header), sync is needed.
